...
def twilio_webhook():
    ...
    # Check if this is the initial interaction
    if 'initial_greeting_done' not in session:
        session['initial_greeting_done'] = True
        session['messages'] = []
        initial_greeting = "Hello, how can I assist you today?"
        return respond_with_tts(initial_greeting)

    # Subsequent interactions: Process user's speech input
    recording_url = request.values.get('RecordingUrl', '')
    if recording_url:
        user_input = get_text_from_speech(recording_url)  # Azure STT
    else:
        user_input = request.values.get('SpeechResult', '').strip()

    # Process user input with chatbot logic
    ...
    return respond_with_tts(chatbot_reply)

def respond_with_tts(text):
    # Convert text to speech using Azure TTS and return a Twilio response
    audio_url = text_to_speech(text)  # Ensure this returns the URL of the audio file
    response = VoiceResponse()
    response.play(audio_url)
    # Removed transcribe=True and transcribe_callback to prevent GET request
...
def text_to_speech(text):
    filename = "C:/projects/Phone_chatbots/Gabby_V2-VOX-1/data/chatbot_response.wav"
    ...
    if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
        ...
        public_url = 'http://4b4a-2600-1700-9580-9e7f-9d02-abd5-aa4a-22d7.ngrok-free.app /audio-response'
        return public_url
    else:
...
def serve_audio_file():
    directory = 'C:/projects/Phone_chatbots/Gabby_V2-VOX-1/data'
...
@app.route("/twilio-transcription-callback", methods=['GET'])
def twilio_transcription_callback():
    # Process the transcription result from Twilio
    transcription_text = request.args.get('TranscriptionText', '').strip()
    
    # Add additional logic as needed to handle the transcription
    # ...
