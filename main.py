...
def twilio_webhook():
    ...
    # If it's the first interaction, greet the user and wait for their response
    if 'initial_greeting_done' not in session:
        session['initial_greeting_done'] = True
        session['messages'] = []
        greeting = "Hello, how can I assist you today?"
        return respond_with_tts(greeting, initial=True)
    ...
    if not user_input:
        # If there's no user input, keep waiting for it
        return str(VoiceResponse())

    # Handle the chatbot logic and respond
    chatbot_reply = process_chatbot_input(user_input)
    return respond_with_tts(chatbot_reply)



def respond_with_tts(text, initial=False):
    audio_url = text_to_speech(text)
    response = VoiceResponse()

    if audio_url.startswith("Error"):
        response.say("I'm sorry, I'm having trouble responding right now.", voice='alice')
    else:
        response.play(audio_url)
        gather = Gather(input='speech', action=url_for('twilio_webhook', _external=True), method='POST')
...
def text_to_speech(text):
    ...
    if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
        ...
        public_url = 'https://cde8-2600-1700-9580-9e7f-400e-76da-fdd3-c04.ngrok-free.app/audio-response'
        return public_url
    else:
...
def serve_audio_file():
    directory = 'C:/projects/Phone_chatbots/Gabby_V2-VOX-1/data' # Use relative path for better portability
